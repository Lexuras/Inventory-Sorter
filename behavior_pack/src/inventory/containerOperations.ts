import type { Block, Container, ItemStack, Player } from "@minecraft/server";
import { sortStacks, type SortableStack } from "./itemSorting.js";
...
export function getBlockInventoryContainer(block: Block): Container | undefined {
  const component = block.getComponent("minecraft:inventory") as { container?: Container } | undefined;
  return component?.container;
}

export function getPlayerInventoryContainer(player: Player): Container | undefined {
  const component = player.getComponent("minecraft:inventory") as { container?: Container } | undefined;
  return component?.container;
}
...
export function isSupportedContainerBlock(block: Block): boolean {
  try {
...
export function sortContainer(player: Player, block: Block): ActionResult {
...
export function sortPlayerInventory(player: Player): ActionResult {
...
export function quickStack(player: Player, block: Block): ActionResult {
  const playerContainer = requireContainer(getPlayerInventoryContainer(player), "player inventory");
  const blockContainer = requireContainer(getBlockInventoryContainer(block), "container");
  const inventoryBefore = readSlots(playerContainer);
  const containerBefore = readSlots(blockContainer);
...
export function depositAll(player: Player, block: Block): ActionResult {
  const playerContainer = requireContainer(getPlayerInventoryContainer(player), "player inventory");
  const blockContainer = requireContainer(getBlockInventoryContainer(block), "container");
  const inventoryBefore = readSlots(playerContainer);
  const containerBefore = readSlots(blockContainer);
...
export function lootAll(player: Player, block: Block): ActionResult {
  const playerContainer = requireContainer(getPlayerInventoryContainer(player), "player inventory");
  const blockContainer = requireContainer(getBlockInventoryContainer(block), "container");
  const inventoryBefore = readSlots(playerContainer);
  const containerBefore = readSlots(blockContainer);
...
export function restock(player: Player, block: Block): ActionResult {
  const playerContainer = requireContainer(getPlayerInventoryContainer(player), "player inventory");
  const blockContainer = requireContainer(getBlockInventoryContainer(block), "container");
  const inventoryBefore = readSlots(playerContainer);
  const containerBefore = readSlots(blockContainer);
...
function commitTransaction(targets: TransactionTarget[]): ActionResult {
...
function sortAndPack(slots: Slot[]): Slot[] {
  const merged = packStacks(slots);
  const sorted = sortStacks(merged);
  const output: Slot[] = sorted.map(({ item }) => item.clone());
...
function packStacks(slots: Slot[]): SortableStack[] {
  const stacks: SortableStack[] = [];
...
function insertIntoSlots(source: ItemStack, targetSlots: Slot[], allowEmptySlots: boolean, slotOrder = orderedSlots(targetSlots.length)): Slot {
  let remaining: Slot = source.clone();
...
function mergeIntoExistingStack(source: ItemStack, target: ItemStack): Slot {
...
function readSlots(container: Container): Slot[] {
  const slots: Slot[] = [];
...
function writeSlots(container: Container, slots: Slot[]): void {
  for (let slot = 0; slot < container.size; slot++) {
...
function canStack(left: ItemStack, right: ItemStack): boolean {
...
function getMaxAmount(item: ItemStack): number {
  return Math.max(1, item.maxAmount ?? item.amount);
}

function sameSlots(left: Slot[], right: Slot[]): boolean {
...
function sameItem(left: Slot, right: Slot): boolean {
  if (!left && !right) {
    return true;
  }
...
function loreKey(item: ItemStack): string {
  try {
...
function countChangedSlots(before: Slot[], after: Slot[]): number {
  let changed = 0;
...
function totalAmount(slots: Slot[]): number {
  return slots.reduce((sum, item) => sum + (item?.amount ?? 0), 0);
}

function requireContainer(container: Container | undefined, name: string): Container {
  if (!container) {
